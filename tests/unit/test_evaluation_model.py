...
def test_summarize_evaluations_computes_all_metrics() -> None:
    observations = [
        EvaluationObservation(
            case_id="exact-id",
        ...
        EvaluationObservation(
            ...
            retrieved_job_ids=["candidate:similar", "candidate:match"],
            comparison_count=2,
            latency_seconds=4.0,
            input_tokens=1000,
            output_tokens=100,
        ...
        EvaluationObservation(
            ...
            retrieved_job_ids=["candidate:generic"],
            comparison_count=1,
            latency_seconds=3.0,
            input_tokens=800,
            output_tokens=80,
        ...
        EvaluationObservation(
            ...
            retrieved_job_ids=["candidate:one", "candidate:two"],
            comparison_count=5,
            latency_seconds=5.0,
            input_tokens=2000,
            output_tokens=200,
    ...
    assert round(summary.total_estimated_cost_usd, 4) == 0.0125


def test_summarize_evaluations_reports_uncollected_usage_as_null() -> None:
    ...
    assert summary.matched_job_id_eligible_cases == 0
    assert summary.correct_matched_job_id_cases == 0
    assert summary.matched_job_id_accuracy is None


def test_validate_evaluation_namespace_accepts_prefixed_suffix() -> None:
    assert (
        validate_evaluation_namespace("  evaluation-test-run  ")
        == "evaluation-test-run"
    )
...
@pytest.mark.parametrize(
    "unsafe_namespace",
    ["", "structured-v1", "production", "evaluation-"],
)
def test_validate_evaluation_namespace_rejects_unsafe_namespaces(
    unsafe_namespace: str,
) -> None:
